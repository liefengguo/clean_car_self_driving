...
int main(int argc, char **argv) {
    ...
    uint16_t result =  usMBCRC16(JIAODU,CMD_LENGTH - 2,pucCRCHi ,pucCRCLo);
    JIAODU[CMD_LENGTH - 1] = pucCRCHi;
    JIAODU[CMD_LENGTH - 2] = pucCRCLo;
    ...
    ser.write(JIAODU, CMD_LENGTH);    // 启动发送和接收线程
    std::mutex mutex;
